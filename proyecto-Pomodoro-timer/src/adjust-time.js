...
//     const clickedElement = e.target.closest(".time-adjuster__item, .time-adjuster__more");
 ...
 */
export function setupAdjustTime(timer) {
  // Maneja clics en los elementos de la lista de ajuste rápido
  adjustTimeList.addEventListener("click", (e) => {
    const clickedElement = e.target.closest(".time-adjuster__item, .time-adjuster__more");
    if (!clickedElement) return;
  ...
  // Maneja eventos de teclado dentro del popup
  popupContainer.addEventListener("keydown", (e) => {
    if (e.key !== 'Enter' && e.key !== 'Escape') return;
